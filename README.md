...
- `UMG`
  - 디자인
    ...
    - 화면 공간 - 메인 뷰포트 해상도에 적용되는 HUD 요소를 디자인한다
    - 월드 공간 - 위젯컴포넌트 혹은 프로젝셔닝을 활용하여 액터 위치에 UI를 표시한다
  - 그래프
    - 바인딩 - 매 프레임마다 실행되는 루틴 코드를 작성한다
    - 이벤트 디스패처 - 버큰 클릭, 텍스트블록 커밋 등 이벤트에 기반한 콜백을 제공한다
    - 머티리얼 - 직접 디자인인 스타일셋을 UI에 적용할 수 있다
  - 입력모드
...
- `Slate`
  - 모듈
    ...
    - 공개 설정 - 게임 모듈 및 내부, 외부 모듈 간 공개 설정 및 캡슐화 가능
    - 빌드 - 모듈 단위 빌드를 통해 추가적은 프로젝트 구성에 있어 유연성을 제공
  - UI문법
    - Slate 문법 - 자체적인 UI 구성 문법을 활용하여 계층구조 작성 가능
    - 탭ID - 에디터 구성 요소에 대한 핸들러를 제공하며 커스터마이징이 가능
    - 블루프린트 연동 - 에디터 전용 C++ 코드를 작성하고 제공할 수 있음
  - 위젯 호환성
    - 해상도 지원 - 다양한 디바이스, 다양한 해상도의 뷰포트를 미리보기하며 래핑
    - 에디터 및 게임 적용 - 동일한 상속 계통을 통해 게임 뿐만 아니라 에디터에도 적용 가능
- `Event`
  - 이벤트 기반
    - 딜리게이트 - UI 요소마다 사전 정의된 딜리게이트를 활용하여 유연성이 높은 바인딩
    - 바인딩 - UObject 상속 기반 및 Raw, Lamda 등 다양한 형태 적용
    - 뷰포트 이벤트 - 위젯 생성 혹은 화면에서 분리할 때 실행되는 라이프사이클을 가짐
  - 호환성
    - C++ 호환성 - 계층 구조를 일일이 탐색할 필요 없이 변수명을 키값으로 바인딩
    - 직렬성 - C++, 블루프린트 양 측의 이벤트는 모두 호환이 가능한 오버라이더블 및 상호 바인딩이 가능
    - 클라이언트 전용 - 모든 이벤트의 클라이언트 RPC화로 
---
## 4. 인공지능 프로그래밍
---
## 5. 플랫폼 프로그래밍
